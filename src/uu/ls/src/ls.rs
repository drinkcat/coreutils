...
fn display_item_name(
    ...
    out: &mut BufWriter<Stdout>,
    style_manager: &mut Option<StyleManager>,
    current_column: impl Fn() -> usize,
) -> OsString {
    // This is our return value. We start by `&path.display_name` and modify it along the way.
    let mut name = escape_name(&path.display_name, &config.quoting_style);

    // On the first call of `is_wrap`, save the value of `current_column` in `cur_col`.
    let mut cur_col: Option<usize> = None;
    let mut is_wrap = |namelen: usize| {
        if config.width == 0 {
            return false;
        };
        if cur_col.is_none() {
            cur_col = Some(current_column()); // This may be expensive to evaluate.
        }
        cur_col.unwrap() + namelen > config.width.into()
    };
